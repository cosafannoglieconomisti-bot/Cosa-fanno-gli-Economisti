...
def add_video_to_playlist(youtube, playlist_id, video_id):
    try:
        # 1. Recupera dettagli video per verificare durata (Esclusione Shorts)
        v_resp = youtube.videos().list(part="contentDetails", id=video_id).execute()
        if not v_resp.get("items"):
            print(f"Video {video_id} non trovato.")
            return False
            
        duration_raw = v_resp["items"][0]["contentDetails"]["duration"]
        duration_seconds = parse_duration(duration_raw)
        
        if duration_seconds <= 120:
            print(f"⚠️ Esclusione Short: Il video {video_id} ({duration_seconds}s) è considerato uno Short/Vertical e non verrà aggiunto alla playlist.")
            return False

        # 2. Controlla se è già presente
        request = youtube.playlistItems().list(
            part="snippet",
            playlistId=playlist_id,
            videoId=video_id
...
def main():
    ...
    if success:
        ...
        print(f"❌ Fallimento catalogazione per {video_key}")
        sys.exit(1)
