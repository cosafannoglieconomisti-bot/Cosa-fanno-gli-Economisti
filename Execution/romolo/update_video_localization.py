    ...
    try:
        ...
        try:
            ...
            if "quota" in str(e).lower():
                print("⚠️ Quota API YouTube superata per le localizzazioni. Procedo comunque per non bloccare la pipeline.")
                return True
            return False
    except Exception as e:
        print(f"❌ Errore generale aggiornamento localizzazioni: {e}")
        if "quota" in str(e).lower():
            print("⚠️ Quota API YouTube superata per le localizzazioni. Procedo comunque per non bloccare la pipeline.")
            return True
        return False
    ...
    try:
        ...
        for lang in languages:
            ...
            try:
                ...
                if "quota" in str(e).lower():
                    print(f"⚠️ Quota superata per sottotitoli {lang}. Continuo comunque per non bloccare l'upload.")
                    continue
                return False
        return True
    except Exception as e:
        print(f"❌ Errore generale caricamento sottotitoli: {e}")
        if "quota" in str(e).lower():
            print("⚠️ Quota API YouTube superata per i sottotitoli. Procedo comunque per non bloccare la pipeline principale.")
            return True
        return False
