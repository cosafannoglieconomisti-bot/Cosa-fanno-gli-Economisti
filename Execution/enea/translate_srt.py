...
def translate_srt(input_path, output_path, target_lang):
    ...
    for attempt in range(7): # Increased to 7 attempts
        try:
            ...
            # Tenta prima con il modello di default, in caso di 503 fall-back sul nuovo gemini-2.5-flash
            ...
            try:
                ...
                if "503" in str(e_inner) or "experience high demand" in str(e_inner).lower() or "429" in str(e_inner) or "quota" in str(e_inner).lower():
                    print("⚠️ Modello primario sovraccarico (503). Provo fall-back su gemini-2.5-flash...", flush=True)
