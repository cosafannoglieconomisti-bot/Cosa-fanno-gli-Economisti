...
def run_captions_upload(video_id, video_path):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"❌ ERRORE CRITICO durante il caricamento sottotitoli: {result.stderr}")
            return False
    except Exception as e:
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Step: Validazione Asset Multilingua (MANDATORIO: IT, EN, ES, FR, DE)
        project_root = os.path.dirname(args.video)
        intl_path = os.path.join(project_root, "international")
        missing = []
        
        print(f"🔍 Avvio validazione asset per il progetto in: {project_root}")
        
        # 1. Verifica Metadati e SRT in Italiano (Root)
        it_meta = os.path.join(project_root, "video_metadata.md")
        it_srt = os.path.join(intl_path, "subtitles_it.srt")
        if not os.path.exists(it_meta): missing.append("Metadata IT (video_metadata.md)")
        if not os.path.exists(it_srt): missing.append("SRT IT (international/subtitles_it.srt)")
        
        # 2. Verifica Metadati e SRT Internazionali
        langs = ["en", "es", "fr", "de"]
        for l in langs:
            meta_f = os.path.join(intl_path, l, f"metadata_{l}.md")
            srt_f = os.path.join(intl_path, l, f"subtitles_{l}.srt")
            if not os.path.exists(meta_f): missing.append(f"Metadata {l.upper()}")
            if not os.path.exists(srt_f): missing.append(f"SRT {l.upper()}")
        
        if missing:
            print("\n" + "!" * 50)
            print("❌ ERRORE CRITICO: ASSET MULTILINGUA MANDATORI MANCANTI")
            print("L'upload è stato BLOCCATO perché mancano i seguenti file:")
            for m in missing:
                print(f"  - {m}")
            print("\nEsegui nuovamente il workflow di `/pulizia` per generare gli asset mancanti.")
            print("!" * 50 + "\n")
            sys.exit(1)
        
        print("✅ Tutti gli asset multilingua (IT, EN, ES, FR, DE) sono presenti. Procedo con l'upload.")
        ...
        success_caps = run_captions_upload(video_id, args.video)
        if not success_caps:
            print("❌ ERRORE: Fallimento caricamento sottotitoli mandatori. Verificare international/ folder.")
            sys.exit(1)
        ...
        if folder_name:
            ...
            if cat_res.returncode != 0:
                print("❌ ERRORE: Fallimento catalogazione playlist. Verificare log di catalog_video.py.")
                sys.exit(1)
            print("✅ Catalogazione playlist completata con successo.")
