...
def cleanup_project(project_name, dry_run=False):
    ...
    # 4. Root Cleanup (Delete any .mp4 or .wav in BASE_DIR)
    print(f"🧹 Scanning root directory for leftovers...")
    root_patterns = ['*.mp4', '*.wav', '*.pdf']
    for pattern in root_patterns:
        for file_path in glob.glob(os.path.join(BASE_DIR, pattern)):
            filename = os.path.basename(file_path)
            # Only delete if it matches the project name (partial or full)
            if project_name.lower() in filename.lower() or "download" in filename.lower():
                print(f"🗑️ Deleting from Root: {filename}")
                if not dry_run:
                    os.remove(file_path)

    # 5. Final Audit (Logging status)
    if not dry_run:
        import subprocess
        try:
