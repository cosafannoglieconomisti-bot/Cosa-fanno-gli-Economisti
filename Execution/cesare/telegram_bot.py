...
sys.path.append("/Users/<USER>/Desktop/canale/Execution/ulisse")
sys.path.append("/Users/<USER>/Desktop/canale/Execution/enea")
...
CESARE_DIR = "/Users/<USER>/Desktop/canale/Execution/cesare"
ASSETS_DIR = "/Users/<USER>/Desktop/canale/Temp/assets"
OVERRIDE_PATH = os.path.join(ASSETS_DIR, "override_cover.png")
HUB_PATH = "/Users/<USER>/Desktop/canale/Temp/cesare/notifications_hub.json"
MAP_PATH = os.path.join(CESARE_DIR, "command_map.json")
BACKLOG_PATH = "/Users/<USER>/Desktop/canale/Temp/cesare/backlog.txt"
BRIDGE_LOG = "/Users/<USER>/Desktop/canale/Temp/cesare/telegram_bridge.log"
PYTHON_PATH = "/Users/<USER>/Desktop/canale/.venv/bin/python3"

user_sessions = {}  # {chat_id: {"paper": "...", "titles": []}}
CACHE_PATH = "/Users/<USER>/Desktop/canale/Temp/cesare/upload_cache.json"
...
def run_and_get_output(key, report_path=None):
    ...
    try:
        ...
        result = subprocess.run(commands[key], capture_output=True, text=True, cwd="/Users/<USER>/Desktop/canale", stdin=subprocess.DEVNULL)
        if report_path and os.path.exists(report_path):
...
def run_and_get_output_with_arg(key, arg):
    ...
    try:
        ...
        cmd.append(arg)
        result = subprocess.run(cmd, capture_output=True, text=True, cwd="/Users/<USER>/Desktop/canale", stdin=subprocess.DEVNULL)
        return result.stdout if result.returncode == 0 else result.stderr
    except Exception as e:
        return f"❌ Errore Subprocess: {e}"
...
def generate_cover_native(title):
    ...
    cmd = [
        ...
        "/Users/<USER>/Desktop/canale/Execution/enea/generate_cover.py",
        title, tmp_path
    ]
    try:
...
def clear_temp_cover(delete_override=False):
    ...
    temp_p = "/Users/<USER>/Desktop/canale/Temp/enea/temp_cover.png"
...
def handle_message(message):
    ...
    if user_msg_stripped.startswith('/'):
        ...
        if cmd == '/backup':
            ...
            def run_gmail():
                ...
                res = run_and_get_output('gmail', report_path="/Users/<USER>/Desktop/canale/Temp/mercurio/gmail_report.txt")
            ...
            def run_report():
                ...
                rep_path = f"/Users/<USER>/Desktop/canale/Temp/romolo/analytics_reports/analytics_report_{time.strftime('%d_%m_%Y')}.txt"
            ...
            def run_articoli():
                ...
                try:
                    ...
                    verify_script = "/Users/<USER>/Desktop/canale/Execution/ulisse/verify_paper.py"
                    ...
                    report_path = f"/Users/<USER>/Desktop/canale/Temp/ulisse/{report_name}"
            ...
            def run_cover_menu():
                try:
                    cleaned_root = "/Users/<USER>/Desktop/canale/Cleaned"
                    folders_with_time = []
            ...
            bot.send_chat_action(message.chat.id, 'typing')
            def run_competitor():
                reply_and_log(message, "⏳ Scansione competitor in corso... (potrebbe richiedere qualche minuto)")
                log_to_bridge("Avvio ricerca Competitor", username, source="Telegram")
                res = run_and_get_output('competitor', report_path="/Users/<USER>/Desktop/canale/Temp/romolo/competitor_engagement.md")
                send_and_log(message.chat.id, res[:4000])
                log_to_bridge("Competitor Report Generato", "Cesare (System)", source="Bridge")
            threading.Thread(target=run_competitor).start()
            return

        # Fallback per comandi non riconosciuti (evita trigger accidentali dell'IA)
        reply_and_log(message, f"⚠️ Comando `{cmd}` non riconosciuto. Usa `/paper`, `/produzione`, `/pulizia`, `/articoli`, `/backup`, `/gmail`, `/report`, `/copertina`, `/upload`, `/playlist` o `/competitor`.")
    ...
    try:
        ...
        pipe_p = "/Users/<USER>/Desktop/canale/Temp/enea/active_pipeline.json"
...
def handle_callback(call):
    ...
    if data.startswith("cover_select:"):
        ...
        def run_cover_gen():
            try:
                ...
                f_path = os.path.join("/Users/<USER>/Desktop/canale/Cleaned", folder_name)
    ...
    if data.startswith("produzione_select:"):
        ...
        def run_orch():
            try:
                ...
                folder_path = os.path.join("/Users/<USER>/Desktop/canale/Cleaned", folder_name)
                ...
                res = subprocess.check_output([PYTHON_PATH, "/Users/<USER>/Desktop/canale/Execution/enea/notebooklm_orchestrator.py"], env=env, stderr=subprocess.STDOUT).decode()
    ...
    if data.startswith("pulizia_select:"):
        ...
        def run_p():
            commands = load_commands()
            cmd = list(commands['pulizia'])
            cmd.append(vid)
            try:
                res = subprocess.run(cmd, capture_output=True, text=True, cwd="/Users/<USER>/Desktop/canale", stdin=subprocess.DEVNULL)
                status_icon = "✅" if res.returncode == 0 else "❌"
                status_text = "Fine pulizia!" if res.returncode == 0 else "ERRORE Pulizia!"
                out = res.stdout if res.returncode == 0 else res.stderr
                send_and_log(chat_id, f"{status_icon} {status_text}\n\n{out[:2000]}")
            except Exception as e:
                send_and_log(chat_id, f"❌ Errore critico pulizia: {e}")
        ...
        def run_title_generation():
            try:
                paper_path = os.path.join("/Users/<USER>/Desktop/canale/Papers/Da fare", pdf)
        ...
        if session:
            ...
            target_dir = os.path.join("/Users/<USER>/Desktop/canale/Cleaned", clean_title)
            ...
            pipe_data = {
                ...
                "paper": session["paper"],
                "paper_path": os.path.join("/Users/<USER>/Desktop/canale/Papers/Da fare", session["paper"]),
                "metadata": session.get("metadata", {}),
                "academic_title": session.get("metadata", {}).get("real_title", "Paper")
            }
            with open("/Users/<USER>/Desktop/canale/Temp/enea/active_pipeline.json", 'w') as f:
                json.dump(pipe_data, f, indent=4)
            ...
            try:
                ...
                if os.path.exists(pipe_p):
                    ...
                    if os.path.exists(src):
                        ...
                        if paper_name:
                            paper_src = os.path.join("/Users/<USER>/Desktop/canale/Papers/Da fare", paper_name)
        ...
        tdir = os.path.join("/Users/<USER>/Desktop/canale/Cleaned", fid)
        ...
        def run_u():
            ...
            cmd = [
                "/Users/<USER>/Desktop/canale/.venv/bin/python3",
                "/Users/<USER>/Desktop/canale/Execution/enea/youtube_uploader.py",
                vpath, title, meta_p, "--thumbnail", thumb_p, "--schedule", publish_at
            ]
            try:
                res = subprocess.run(cmd, capture_output=True, text=True, cwd="/Users/<USER>/Desktop/canale/Execution/credentials", stdin=subprocess.DEVNULL)
                status_icon = "✅" if res.returncode == 0 else "❌"
                status_text = "Upload completato!" if res.returncode == 0 else "ERRORE Upload!"
                out = res.stdout if res.returncode == 0 else res.stderr
                send_and_log(chat_id, f"{status_icon} {status_text}\n\n{out[:1000]}")
                if res.returncode == 0:
        ...
        def run_playlist():
            ...
            try:
                res = subprocess.run(cmd, capture_output=True, text=True, cwd="/Users/<USER>/Desktop/canale/Execution/romolo", stdin=subprocess.DEVNULL)
                out = res.stdout if res.returncode == 0 else res.stderr
