...
new_logic = """elif user_msg_stripped == '/articoli':
        ...
        try:
            # 1. Recupero Notizie
            prompt_news = "Sei un giornalista. Cerca le 5 notizie (hot topics) più importanti di oggi in Italia (usa ANSA o Repubblica). Ritorna solo un elenco numerato."
            ...
            for i, topic in enumerate(topics[:5], 1):
                clean_topic = topic.split('.', 1)[1].strip() if '.' in topic else topic
                report_lines.append(f"\\n🔥 **ARGOMENTO {i}**: {clean_topic}")
                
                # 2. Ricerca Paper (Uso stringa singola per evitare errori di indentazione nel generatore)
                prompt_paper = "Sei un ricercatore di Economia. Trova 3 articoli accademici correlati a questo tema: \\"" + clean_topic + "\\" da riviste prestigiose dal 2000 ad oggi. Rispondi solo in questo formato (una riga per paper): Titolo | Autori | Journal"
...
print("Logic chained successfully!")
