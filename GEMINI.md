...
1. **Selezione Paper**: L'utente lancia `/paper` su Telegram e seleziona un file da `Papers/Da fare/`.
2. **Generazione Titoli**: Il bot estrae il testo e propone 5 titoli "catchy" (max 5 parole, stile domanda).
3. **Generazione Copertina**: Una volta scelto il titolo, il sistema genera immediatamente una proposta di copertina in stile Comic (Arancio/Nero/Bianco) con il titolo integrato.
4. **Approvazione e Setup**: Se la copertina è approvata, il sistema:
    - Recupera il **Titolo Accademico** reale.
    - Crea la cartella `Cleaned/[Titolo_Video]`.
    - **Sposta e Rinomina** il PDF in `Cleaned/[Titolo_Video]/[Titolo_Accademico].pdf`.
    - Salva `copertina.png` e inizializza `video_metadata.md` (con dati estratti dal PDF) nella cartella.
5. **Titolo Forzato (MANDATORIO)**: Il titolo scelto **DEVE** essere applicato rigorosamente a tutti i contenuti successivi.
...
- **Post-Upload Cleanup (MANDATORIO)**: Una volta confermati gli upload su tutte le piattaforme, **DEVE** essere eseguita la pulizia deterministica (`video_cleanup.py`):
    ...
    - **Update Registro**: Imposta lo stato su `Pulito` in `video_tracking.json`.
...
### SOP 1: Selezione, Titolo e Copertina
...
### SOP 3: Pulizia, Archiviazione e Upload Multi-Piattaforma
1. `Execution/cesare/telegram_bot.py` (Attivazione via `/pulizia` o `/upload`)
2. `Execution/enea/video_processor.py` (Post-processing)
3. `Execution/enea/generate_index_whisper.py` (Trascrizione e Indice)
...
Competente per il monitoraggio, analytics, commenti, salute del canale e scouting competitor.
...
### SOP: Scouting Competitor e Commenting di Alto Valore (Workflow /competitor)

**OBIETTIVO**: Identificare video di competitor/canali affini e proporre commenti di alto valore che citano un paper accademico e linkano un video del nostro canale per convogliare traffico qualificato.

1. **Scansione Ibrida & Quota Saving**:
   - Eseguire lo script `Execution/romolo/competitor_scout.py`.
   - Per proteggere la quota dell'API di YouTube, non cercare i canali target per nome, ma utilizzare la mappa pre-caricata con i **Channel ID fissi** (es. *Starting Finance, Will Media, Geopop, ORA!, Mr. Rip, Limes, WesaChannel, Michele Boldrin, ecc.*).
   - Per diversificare al massimo i canali e i video target senza fossilizzarsi sempre sui medesimi account, eseguire una **Ricerca Semantica per Parole Chiave** campionando casualmente keywords dell'archivio del canale (es. *tasse italia, meritocrazia, pensioni, natalità, gender gap, mafia, criminalità*). Ciò permette di scoprire e commentare video caldi di qualunque creatore in lingua italiana.
...
3. **Pubblicazione ed Archiviazione**:
   - I commenti approvati vengono pubblicati automaticamente sotto i video scelti via API tramite lo script `Execution/romolo/post_youtube_comment.py`.
   - Gli ID dei video commentati vengono salvati nello storico JSON per evitare duplicati in futuro.

---

## 📋 File Python Utilizzati
- `Execution/romolo/romolo_manage_channel.py` (Analytics e Monitoraggio)
- `Execution/romolo/competitor_scout.py` (Scouting competitor)
- `Execution/romolo/post_youtube_comment.py` (Pubblicazione automatica commenti)
...
6. **Esclusione Shorts (MANDATORIO)**: Gli YouTube Shorts (video verticali o con durata <= 120s) non devono mai essere inseriti nelle playlist tematiche. Le playlist sono riservate esclusivamente ai contenuti long-form (documentari e paper review).
...
#### Regola 1 — Script Unico Autorizzato
...
- `/Users/<USER>/Desktop/canale/.venv/bin/python3` (Interprete Mandatorio)
