...
7.  **Caricamento Sottotitoli e Metadati (MANDATORIO)**: Il sistema deve automaticamente caricare le tracce professionali e i metadati localizzati (EN, ES, FR, DE) presenti nella sottocartella `international/` del progetto via `update_video_localization.py`. **L'upload fallisce se mancano gli asset multilingua.**
8.  **Mandatory Cleanup (MANDATORIO)**: Una volta confermato l'upload e la catalogazione in playlist, è **obbligatorio** eseguire la pulizia deterministica tramite lo script `Execution/enea/video_cleanup.py`. Questo passaggio rimuove i file `.mp4` (raw e cleaned), l'infografica raw e il **PDF del paper**, archiviando gli asset rimanenti in `international/` e impostando lo status del progetto a `Pulito`.
